...
def vesicle_3D_dilation(path):
    ...
    # Setting an uncertainty zone outsite the mask of the vesicle.
    labels[mask == 0] = 2
...
def load_training():
    ...
    for file in files:
        ...
        trainingFinal_x.extend(list(training_x))
        trainingFinal_y.extend(list(training_y))

    mask = [np.any(i != 2) for i in trainingFinal_y]
    training_x = [i for i, m in zip(trainingFinal_x, mask) if m]
    training_y = [i for i, m in zip(trainingFinal_y, mask) if m]

    return training_x, training_y



# Change this to load your testing data.
def load_testing():
...
def test_unet(niter, datasets, unet_clsf, hint_patch_shape):
    ...
    for dataset in datasets:
        pred = np.array([unet.predict_in_blocks(unet_clsf, i, hint_patch_shape, 
...
def main(hint_patch_shape=(100,100),          
         learning_rate=1e-4,
         save_path="/cvlabdata2/home/mas/UNet-2D-Test"):    
